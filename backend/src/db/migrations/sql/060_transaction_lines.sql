...
-- RLS: same pattern as transactions (filter via parent's tenant)
ALTER TABLE transaction_lines ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS tl_tenant_iso ON transaction_lines;
CREATE POLICY tl_tenant_iso ON transaction_lines
  USING (
    EXISTS (
      SELECT 1 FROM transactions tx
       WHERE tx.id = transaction_lines.transaction_id
         AND tx.tenant_id = current_setting('app.tenant_id', true)::uuid
    )
  )
  WITH CHECK (
    EXISTS (
